...
class WrapperWALayer(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        # cast to bf16/fp16
        x = x.to(self.orig_layer.weight.dtype)
...
class WrapperLinearForTEQ(torch.nn.Module):
    ...
    def unwrapper(self, v, min_scale=None, max_scale=None):
        # min_scale.clamp_(0, 1.0)
        # max_scale.clamp_(0, 1.0)
        # return the origin layer directly?
        return self.orig_layer

        qdq_weight, scale, zp = quant_tensor(
...
def wrapper_block_teq(block, enable_minmax_tuning):
    ...
    for n, m in block.named_modules():
        if isinstance(m, torch.nn.Linear):
            if not check_to_quantized(m):
                ...
                continue
            new_m = WrapperLinearForTEQ(m, enable_minmax_tuning=enable_minmax_tuning)
