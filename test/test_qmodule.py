...
import torch
import random
from auto_round.qmodules import (
    QuantizerConfig,
    FlexRoundLinear,
    FlexRoundModuleConfig,
    default_quantizer_config,
    ada_default_quantizer_config,
)
...
class TestFlexRoundLinear:
    ...
    def test_flex_linear_group_size_minus_one(self):
        ...
        float_linear, flex_round_linear = self._create_flex_round_linear(
            in_features=in_features, out_features=out_features, quantizer_config=quantizer_config
        )
    ...
    def _create_flex_round_linear_ada(
        cls, in_features=32, out_features=64, quantizer_config=ada_default_quantizer_config
    ):
        config = FlexRoundModuleConfig(weight_config=quantizer_config)
        float_linear = torch.nn.Linear(in_features, out_features)
        flex_round_linear = FlexRoundLinear(float_linear, config=config)
    ...
    def test_ada_forward(self):
        ...
        bs = 4
        iters = 3
        float_linear, flex_round_linear = self._create_flex_round_linear_ada(
            in_features=in_features, out_features=out_features
        )
        dummy_input = torch.randn(bs, in_features)
        for i in range(iters):
